...
func Bind(types []string, abis []string, bytecodes []string, fsigs []map[string]string, pkg string, lang Lang, libs map[string]string, aliases map[string]string) (string, error) {
	var (
		...
		// all events map event id to a list of events
		allEvents = make(map[common.Hash][]*tmplEvent)
	)
	for i := 0; i < len(types); i++ {
		...
		var (
			...
			// identifier collision in the bindings of these categories.
	...
	for h, lev := range allEvents {
		h0, ok := allByName[lev[0].Normalized.Name]
		if ok {
			toRemove[h] = true
			toRemove[h0] = true
			continue

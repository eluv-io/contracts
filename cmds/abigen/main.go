...
func abigen(c *cli.Context) error {
	...
	// output the solc version
	if c.GlobalIsSet(solFlag.Name) {
		sol, _ := compiler.SolidityVersion(c.GlobalString(solcFlag.Name))
		solInfo := fmt.Sprintf("solc: %s\n%s", sol.Path, sol.FullVersion)
		_ = ioutil.WriteFile(
			filepath.Join(
				filepath.Dir(c.GlobalString(outFlag.Name)),
				"solc_version.txt"),
			[]byte(solInfo),
			0600)
	}
